...
- **Generated docs:** every built-in profile and fragment carries `meta.description`; `make docs` (via `cmd/gen-catalog`, `internal/profile/gendocs.go`) rewrites the README profiles table between its `<!-- BEGIN/END tpd profiles -->` markers and `docs/catalog.md`. **Every change to a built-in profile or fragment under `internal/catalog/` requires `make docs`** — `TestDocsUpToDate` and `TestBuiltinsHaveMetaDescriptions` fail otherwise.
